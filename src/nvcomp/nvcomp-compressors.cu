...
get_compressor_manager(const enums_nvcomp::CompressionType compression_type,
                       const nvcompType_t data_type, const size_t chunk_size) {
...
__global__ void check_buffer_equality(const uint8_t *buffer_a,
  ...
  size_t index =
      N_ELEMENTS_PER_THREAD * (threadIdx.x + blockIdx.x * blockDim.x);
...
bool compare_d_buffers(const uint8_t *buffer_a, const uint8_t *buffer_b,
  ...
  check_buffer_equality<<<n_blocks, 1024>>>(buffer_a, buffer_b, length,
                                            d_result);

  CUDA_SAFE_CALL(
      cudaMemcpy(&result, d_result, sizeof(bool) * 1, cudaMemcpyDeviceToHost));
